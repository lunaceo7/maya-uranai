...
const SCENARIOS = [
  {
    ...
    rubric: [
      { text: "相手の感情への共感があるか", tip: "「もどかしいですよね」「不安な気持ちよくわかります」など感情に寄り添う言葉があるか" },
      { text: "状況を整理して伝えているか", tip: "「目が合う」「ランチに誘う」などの事実を整理して伝えられているか" },
      { text: "具体的な行動アドバイスがあるか", tip: "「〇〇してみては」など次の行動が明確に提示できているか" },
  ...
  },
  {
    ...
    rubric: [
      { text: "家族への配慮が入っているか", tip: "妻・子どもへの言及があるか。家族を大切にしている姿勢が伝わるか" },
      { text: "現状の感情に寄り添えているか", tip: "5年間の積み上げへのねぎらい、不安への共感があるか" },
      { text: "転職に対する見立てが明確か", tip: "「今が時期」「もう少し待つべき」など自分の見立てを明示できているか" },
      { text: "次の行動が具体的か", tip: "「まず〇〇から始めてみては」など具体的な第一歩があるか" },
    ],
    sampleAnswer: "○○さん、5年間積み上げてこられたものと、家族への責任感の間で揺れるお気持ち、とてもよく伝わります。あなたの中にはすでに「変わりたい」という強い意志が芽生えています。リスクへの恐れは、それだけ大切なものがある証拠です。まずは転職エージェントへの登録や、副業での市場価値確認など、「辞める前にできること」から始めてみてください。動き出すことで、霧が晴れるように次の道が見えてくるはずです。",
  ...
  },
  {
    ...
    rubric: [
      ...
      { text: "複数の選択肢を示せているか", tip: "「①〇〇②〇〇③〇〇」のように行動の選択肢を示せているか" },
      { text: "相手の状況に合った提案ができているか", tip: "フリーランスという立場・仕事への影響を考慮した提案か" },
      { text: "自分を守ることへの肯定が入っているか", tip: "「あなたは間違っていない」など自己肯定感を高める言葉があるか" },
  ...
  },
  {
    ...
    rubric: [
      ...
      { text: "独立のリスクと備えへの言及があるか", tip: "収入・貯蓄・準備期間など具体的な備えに触れているか" },
      { text: "家族との対話を促せているか", tip: "奥様の気持ちを聞く・話し合うことへの促しがあるか" },
      { text: "「今がタイミングか」への明確な見立てがあるか", tip: "YesかNoかを明確に示しているか（曖昧にしていないか）" },
      { text: "背中を押しつつも慎重さも伝わるか", tip: "希望と現実のバランスが取れているか" },
    ],
    sampleAnswer: "○○さん、副業で月10万円という実績は素晴らしい土台です。ただ、タイミングとしてはもう少しだけ準備期間を取ることをお勧めします。生活費6ヶ月分の貯蓄と、副業収入を月20〜30万円に伸ばすことが安心の基準になるでしょう。また、奥様の「積極的でない」という気持ちの背景を、一度じっくり聞いてみてください。家族の安心が、あなたの力になります。",
    tips: ["数字を使うと信頼感が増す（貯蓄額・収入目標）", "「まだ早い」と言うときも必ず希望を添える", "家族への言及は必須。特に配偶者の気持ちに触れる"],
  },
];
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 10, marginTop: 4 }}>
      <div style={{ flex: 1, height: 6, background: "#1a1a2e", borderRadius: 3, overflow: "hidden" }}>
        <div style={{
          ...
          background: color || (score >= 80 ? "#c9a84c" : score >= 50 ? "#8b9dc3" : "#6b4c4c"),
...
export default function App() {
  ...
  const [phase, setPhase] = useState("select"); // select | write | check | result
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await window.storage.get("uranai-history");
  ...
  const saveHistory = async (newHistory) => {
    try { await window.storage.set("uranai-history", JSON.stringify(newHistory)); } catch (e) {}
  };
  ...
  const currentScore = selectedScenario
    ? Math.round((checkedItems.length / selectedScenario.rubric.length) * 100)
    : 0;
  ...
  return (
    <div style={{
      minHeight: "100vh", background: "#0d0d1a", color: "#e8e0d0",
      fontFamily: "'Georgia', 'Noto Serif JP', serif", paddingBottom: 60
    }}>
      {/* ヘッダー */}
      <div style={{
        background: "linear-gradient(135deg, #1a1030, #0d0d1a, #1a0d20)",
        borderBottom: "1px solid #2a2040", padding: "24px 20px 16px", textAlign: "center"
      }}>
        <div style={{ fontSize: 24, marginBottom: 4 }}>🔮</div>
        <h1 style={{ fontSize: 20, fontWeight: 700, color: "#c9a84c", margin: "0 0 4px", letterSpacing: "0.1em" }}>
          占い師トレーニング
        </h1>
      ...
      {/* ステータスバー */}
      <div style={{
        background: "#100d1a", borderBottom: "1px solid #1a1535",
        ...
        {tab === "history" && (
          <div style={{ paddingTop: 20 }}>
            <div style={{
              ...
              border: `1px solid ${rank.color}40`, borderRadius: 12, padding: 20, marginBottom: 14
            }}>
                ...
                <div>
                  <div style={{ height: 6, background: "#1a1a2e", borderRadius: 3, overflow: "hidden" }}>
                    <div style={{
                      ...
                      width: `${Math.min(((totalCount - rank.min) / (nextRank.min - rank.min)) * 100, 100)}%`,
                      transition: "width 1s ease"
                    }} />
                  </div>
                  <div style={{ fontSize: 10, color: "#6b6058", marginTop: 3, textAlign: "right" }}>
                    {totalCount - rank.min} / {nextRank.min - rank.min}
                  </div>
                </div>
              )}
            </div>
            ...
            <div style={{ background: "#100d1a", border: "1px solid #2a2040", borderRadius: 12, overflow: "hidden" }}>
              ...
              {history.length === 0 ? (
                <div style={{ padding: 30, textAlign: "center", color: "#5a5050", fontSize: 13 }}>
                  まだ練習記録がありません
                </div>
              ) : [...history].reverse().slice(0, 10).map((h) => (
                <div key={h.id} style={{
                  padding: "11px 16px", borderBottom: "1px solid #1a1535",
                  ...
                  <div style={{
                    ...
                    color: h.score >= 80 ? "#c9a84c" : h.score >= 50 ? "#8b9dc3" : "#8b6060"
                  }}>{h.score}点</div>
                </div>
              ))}
            </div>
                ...
                <div style={{ display: "flex", gap: 8, padding: "16px 0 12px", justifyContent: "center" }}>
                  {["すべて", "初心者", "上級者"].map(l => (
                    <button key={l} onClick={() => setFilterLevel(l)} style={{
                      ...
                      color: filterLevel === l ? "#c9a84c" : "#8b8070",
                    ...
                    return (
                      <button key={s.id} onClick={() => { setSelectedScenario(s); setPhase("write"); }}
                        style={{
                          ...
                          padding: "16px 18px", textAlign: "left", cursor: "pointer", color: "#e8e0d0"
                        }}
            ...
            {phase === "write" && selectedScenario && (
              <div style={{ paddingTop: 16 }}>
                <button onClick={reset} style={{
                  background: "none", border: "none", color: "#8b8070",
                  cursor: "pointer", fontSize: 12, padding: "0 0 12px"
                }}>← 一覧へ</button>

                {/* ステップ表示 */}
                <div style={{ display: "flex", gap: 0, marginBottom: 16, background: "#100d1a", borderRadius: 10, overflow: "hidden", border: "1px solid #2a2040" }}>
                ...
                <div style={{
                  background: "linear-gradient(135deg, #1a1530, #160d20)",
                  border: "1px solid #2a2040", borderRadius: 12, padding: 18, marginBottom: 12
                }}>
                  ...
                  <div style={{
                    ...
                    fontSize: 13, lineHeight: 1.8, color: "#c8c0b0", borderLeft: "3px solid #c9a84c"
                  }}>{selectedScenario.situation}</div>
                </div>

                <div style={{
                  background: "rgba(139,157,195,0.05)", border: "1px dashed #3a3860",
                  borderRadius: 8, padding: 10, marginBottom: 12, fontSize: 12, color: "#8b9dc3", lineHeight: 1.6
                }}>💡 {selectedScenario.hint}</div>

                <textarea
                  value={userAnswer}
                  onChange={e => setUserAnswer(e.target.value)}
                  placeholder="お客様へのテキスト鑑定文をここに書いてください…"
                  style={{
                    width: "100%", minHeight: 160, background: "#0d0d1a",
                    border: "1px solid #2a2040", borderRadius: 10, padding: 12,
                    color: "#e8e0d0", fontSize: 13, lineHeight: 1.8,
                    resize: "vertical", fontFamily: "inherit", boxSizing: "border-box", outline: "none",
                    marginBottom: 6
                  }}
                  onFocus={e => e.target.style.borderColor = "#c9a84c"}
                  onBlur={e => e.target.style.borderColor = "#2a2040"}
                />
                ...
                <button onClick={() => setPhase("check")} disabled={!userAnswer.trim()} style={{
                  ...
                  color: userAnswer.trim() ? "#0d0d1a" : "#4a4060",
              ...
              <div style={{ paddingTop: 16 }}>
                ...
                <div style={{ display: "flex", gap: 0, marginBottom: 16, background: "#100d1a", borderRadius: 10, overflow: "hidden", border: "1px solid #2a2040" }}>
                  {["① 鑑定文を書く", "② 自己チェック", "③ 結果"].map((s, i) => (
                    <div key={i} style={{
                      flex: 1, padding: "8px 4px", textAlign: "center", fontSize: 11,
                      background: i === 1 ? "rgba(201,168,76,0.1)" : "transparent",
                      color: i === 1 ? "#c9a84c" : "#4a4060",
                      borderRight: i < 2 ? "1px solid #2a2040" : "none"
                    }}>{s}</div>
                  ))}
                </div>

                <div style={{ fontSize: 13, color: "#c8c0b0", marginBottom: 12, lineHeight: 1.7 }}>
                  自分の鑑定文を見ながら、該当するものにチェックを入れてください。
                </div>

                {/* 自分の鑑定文（折りたたみ） */}
                <details style={{ marginBottom: 14 }}>
                  <summary style={{ cursor: "pointer", fontSize: 12, color: "#8b8070", padding: "8px 0" }}>
                    📝 自分の鑑定文を確認する
                  </summary>
                  <div style={{
                    background: "rgba(0,0,0,0.3)", borderRadius: 8, padding: 12,
                    fontSize: 13, lineHeight: 1.8, color: "#c8c0b0",
                    borderLeft: "3px solid #2a2040", marginTop: 8
                  }}>{userAnswer}</div>
                </details>
                  ...
                  {selectedScenario.rubric.map((r, i) => (
                    <button key={i} onClick={() => toggleCheck(i)} style={{
                      ...
                      <div style={{
                        ...
                        border: `2px solid ${checkedItems.includes(i) ? "#c9a84c" : "#3a3060"}`,
                        background: checkedItems.includes(i) ? "#c9a84c" : "transparent",
                        display: "flex", alignItems: "center", justifyContent: "center",
                        fontSize: 13, color: "#0d0d1a", fontWeight: 700, marginTop: 1
                      }}>
                        {checkedItems.includes(i) ? "✓" : ""}
                      </div>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: 13, color: checkedItems.includes(i) ? "#c9a84c" : "#c8c0b0", marginBottom: 3 }}>
                          {r.text}
                        </div>
                ...
                <div style={{
                  ...
                  borderRadius: 10, padding: 12, marginBottom: 14, textAlign: "center"
                }}>
                  <div style={{ fontSize: 11, color: "#8b8070", marginBottom: 4 }}>現在のスコア</div>
                  <div style={{ fontSize: 32, fontWeight: 700, color: "#c9a84c" }}>
                    {checkedItems.length} / {selectedScenario.rubric.length}
                  </div>
                  <ScoreBar score={currentScore} />
                </div>

                <button onClick={submitSelfCheck} style={{
                  width: "100%", padding: "13px",
                  background: "linear-gradient(135deg, #c9a84c, #a07830)",
                  border: "none", borderRadius: 10, color: "#0d0d1a",
              ...
              <div style={{ paddingTop: 16 }}>
                ...
                <div style={{ display: "flex", gap: 0, marginBottom: 16, background: "#100d1a", borderRadius: 10, overflow: "hidden", border: "1px solid #2a2040" }}>
                  {["① 鑑定文を書く", "② 自己チェック", "③ 結果"].map((s, i) => (
                    <div key={i} style={{
                      flex: 1, padding: "8px 4px", textAlign: "center", fontSize: 11,
                      background: i === 2 ? "rgba(201,168,76,0.1)" : "transparent",
                      color: i === 2 ? "#c9a84c" : "#4a4060",
                      borderRight: i < 2 ? "1px solid #2a2040" : "none"
                    }}>{s}</div>
                  ))}
                </div>
                ...
                <div style={{
                  background: "linear-gradient(135deg, #1a1530, #160d20)",
                  border: "1px solid #c9a84c", borderRadius: 12, padding: 24,
                  textAlign: "center", marginBottom: 14
                }}>
                  <div style={{ fontSize: 11, color: "#8b8070", marginBottom: 6, letterSpacing: "0.15em" }}>スコア</div>
                  <div style={{ fontSize: 52, fontWeight: 700, color: "#c9a84c", textShadow: "0 0 30px rgba(201,168,76,0.4)" }}>
                    {currentScore}
                  </div>
                ...
                <div style={{ background: "#100d1a", border: "1px solid #2a2040", borderRadius: 12, padding: 16, marginBottom: 16 }}>
                  ...
                  <div style={{
                    fontSize: 13, lineHeight: 1.9, color: "#c8c0b0",
                    borderLeft: "3px solid #8b9dc3", paddingLeft: 12
                  }}>{selectedScenario.sampleAnswer}</div>
                </div>

                <div style={{ display: "flex", gap: 10 }}>
                  <button onClick={() => { setPhase("write"); setCheckedItems([]); setUserAnswer(""); }} style={{
                    flex: 1, padding: "12px", background: "transparent",
                    border: "1px solid #2a2040", borderRadius: 10, color: "#8b8070", fontSize: 13, cursor: "pointer"
                  }}>↩ もう一度</button>
                  <button onClick={() => { reset(); setTab("history"); }} style={{
                    flex: 1, padding: "12px",
                    background: "linear-gradient(135deg, #c9a84c, #a07830)",
                    border: "none", borderRadius: 10, color: "#0d0d1a", fontSize: 13, fontWeight: 700, cursor: "pointer"
                  }}>📊 成長記録を見る</button>
                </div>
              </div>
            )}
